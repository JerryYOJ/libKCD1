...
// <cstdint>, <cstddef>, <new>, <vector> come from PCH.h.

namespace wh
{
	namespace shared
	{
		// External inline buffer an InplaceVector allocates from — a sibling of the vector,
		// NOT embedded in the allocator (the allocator merely points here).
		template <class T, int N>
		struct InplaceStorage
		{
			T    buffer[N];  // 00            inline element storage
			bool inUse;      // N*sizeof(T)   buffer currently handed out?
			T*   self;       //               = buffer
		};

		// Thin stateful allocator: a pointer to an InplaceStorage (Howard Hinnant short_alloc
		// idiom — std::pmr-style but non-virtual). Serves the inline buffer while it fits and
		// is free, else the heap; deallocate frees heap blocks only. Matches sub_180453848 /
		// sub_18066EAD4. Non-empty, so the owning vector keeps a leading allocator slot.
		template <class T, int N>
		struct InplaceAllocator
		{
			...
			struct rebind
			{
				using other = InplaceAllocator<U, N>;
			};

			InplaceStorage<T, N>* storage;  // 00  -> the arena (null = always heap)

			InplaceAllocator() noexcept :
				storage(nullptr)
			{}
			explicit InplaceAllocator(InplaceStorage<T, N>* a_storage) noexcept :
				storage(a_storage)
			{}
			template <class U>
			...
			{
				if (storage && a_ptr == storage->buffer) {
					storage->inUse = false;
				} else {
		...
		// std::vector<T> with inline storage for N elements (small-buffer optimization): the
		// arena + a std::vector whose stateful allocator points back at it. Non-copyable (the
		// allocator is self-referential, like Hinnant's arena). Layout VERIFIED (sub_180453848
		// / sub_181079778 = MSVC std::vector reallocate); sizeof = N*sizeof(T) + 0x30.
		template <class T, int N>
		struct InplaceVector
		{
			InplaceStorage<T, N>                   _storage;  // 00
			std::vector<T, InplaceAllocator<T, N>> _vec;      //    { alloc(-> _storage), first, last, end }

			InplaceVector() :
				_vec(InplaceAllocator<T, N>{ &_storage })
			{
