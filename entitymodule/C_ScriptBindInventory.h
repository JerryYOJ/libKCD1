...
public:
    ...
    void*                   m_pEntitySystem;    // +0x68  IEntitySystem* (gEnv->pEntitySystem)

    // Lua methods (18)
    int AddItem(IFunctionHandler* pH, SmartScriptTable item);                       // 0x1810BDF68
    int RemoveItem(IFunctionHandler* pH, EntityId id, int count);                   // via thunk
    int DeleteItem(IFunctionHandler* pH, EntityId id, int count);                   // 0x1810C267C
    int DeleteItemOfClass(IFunctionHandler* pH, const char* guid, int count);       // 0x1810C2690
    ...
    int Validate(IFunctionHandler* pH);                                             // 0x1810EDCD0
