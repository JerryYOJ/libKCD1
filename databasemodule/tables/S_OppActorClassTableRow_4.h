...
// Registration: 0x1800bff00
#pragma pack(push, 1)
struct S_OppActorClassTableRow_4 {
    ...
    bool broken_block;  // 0x0E8
    bool is_weapon_hit;  // 0x0E9
    uint8_t _pad_0x0EA[2];
    int32_t hit_origin;  // 0x0EC
    const char* hit_mn_fragment_id;  // 0x0F0
    uint8_t _pad_0x0F8[8];
    const char* hit_mn_tags;  // 0x100
    // --- virtual columns (offset=-1, not in struct memory) ---
    // int32_t actor_class_hash;
};
#pragma pack(pop)
static_assert(sizeof(S_OppActorClassTableRow_4) == 264, "Size mismatch for S_OppActorClassTableRow_4");
